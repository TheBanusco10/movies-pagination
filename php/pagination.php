...
$statementt = $db_con->prepare("select * from movies where id > :id");
$statementt->execute(array(':id' => 0));

$list['peliculas'] = $statementt->fetchAll(PDO::FETCH_ASSOC);

$limit = 10;

$total_records = count($list['peliculas']);
...
$statement = $db_con->prepare("select * from movies ORDER BY id ASC LIMIT $start_from, $limit");
$statement->execute();
